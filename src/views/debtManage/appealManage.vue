<template>
    <div class="app-container calendar-list-container ggg_user">
        ...
        <el-form :inline="true" :model="params" ref="params" class="demo-form-inline" @keyup.enter.native="getTable('isseach')">
            ...
            <el-form-item label="申诉时间" prop="date">
        ...
        <div class="filter-container">
            <el-table ref="multipleTable" :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
                ...
                <el-table-column align="center" label="申诉时间" width="150">
                    <template slot-scope="scope">
                        {{scope.row.appealTime}}
                    </template>
                </el-table-column>
                ...
                <!--<el-table-column align="center" label="申诉结果" width="140">
                                                                                                                                                                            <template slot-scope="scope">
                                                                                                                                                                                <span v-for="item in status" :key="item.value" v-text="item.key" v-show="item.value == scope.row.status"></span>
                                                                                                                                                                            </template>
                                                                                                                                                                        </el-table-column>-->
                <el-table-column align="center" label="操作" fixed="right" width="200">
                    <template slot-scope="scope">
                        ...
                        <el-button type="primary" size="mini" @click="lookOver(scope.row, 1)" v-show="scope.row.status == 20">申诉审核</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <el-dialog :title="title" :visible.sync="dialogVisible" width="47%">
            <el-form :rules="rules" :model="alertFrom" ref="alertFrom" label-width="155" @keyup.enter.native="submitForm('alertFrom')">
                ...
                <div v-if="edit||alertFrom.status == 22">
                    <el-row>
                        <el-form-item label="选择审核结果" prop="status1">
                            <el-select v-model="alertFrom.status1" placeholder="请选择" :disabled="!edit" style="width:100%;">
...
export default {
    ...
    methods: {
        ...
        download() {
            downloadExecl({ url: 'tOrderTransfer/exportAppeal', input: this.params, method: 'get' })
        },
        lookOver(row, number) {
